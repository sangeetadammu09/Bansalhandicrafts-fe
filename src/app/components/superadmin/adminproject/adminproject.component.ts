...
export class AdminprojectComponent {
  ...
  breadcrumb = ['Dashboard', 'Admin', 'Project List'];
  startNumber = 1;
  pageSize = 10;
  productList: any[] = [];
  filterText = '';
  ...
  getProducts() {
    ...
    this.projectService.listOfProjects(this.filterPayload).subscribe({next: (data: any) => {
        if (data.status == 200) {
  ...
  submit() {
    ...
    if (this.productForm.valid) {
      ...
      if (this.productId == '0') {
        ...
        this.projectService.updateProject(this.productId, formData).subscribe({
          next: (data: any) => {
            if (data.status == 200) {
              //  $('#parentModal').modal('show')
              this.getProducts();
            ...
            if (error.status == 500) {
              this.toastrService.error('Server Error.Failed to update data');
            }
  ...
  action(item: any, type: any) {
    if (type == 'view') {
      this.modaltitle = 'Project Images';
  ...
  deleteProduct(){
    this.projectService.deleteProject(this.productId).subscribe({next: (data:any)=>{
      if(data.status == 200){
